...
CONST Author = "H. G. Wells" 'If SetAuthorName is -1 then this must be set to the name you want to use for the author.  Else, it can be anything as it's ignored.
CONST SaveChanges = -1 '-1 to make the actual changes, 0 to just preview them to see how things will look.
...
DO UNTIL EOF(1)
    ...
    of$ = f$
    IF StripUnderScores THEN Remove_US f$ 'Remove underscores from the booktitle if we have the CONST set so we do so.
    ...
    IF h OR SetAuthorName THEN
        b$ = a$ + " - " + LTRIM$(RTRIM$(t$)) 'The complete book name now.  "Stephen King - Book Title.txt"
    ELSE
        b$ = LTRIM$(RTRIM$(f$)) 'We had no hyphon, and we're not setting an author of our own, so the title is just the title with perhaps a few corrections.
    END IF
    IF ChangeFolder THEN b$ = LTRIM$(RTRIM$(a$))
    IF AddAuthor THEN b$ = Author$ + " - " + LTRIM$(RTRIM$(f$))

    IF SetLowerCaseExtension THEN MID$(b$, LEN(b$) - 2, 3) = LCASE$(MID$(b$, LEN(b$) - 2, 3)) 'make the extension lowercase if the flag is set.


    f$ = Dir$ + of$: b$ = Dir$ + b$ 'Add in the directory path before renaming the file
